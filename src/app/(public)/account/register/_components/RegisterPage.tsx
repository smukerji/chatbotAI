...
function RegisterPage() {
  ...
  // Uniform message logic as login page
  useEffect(() => {
    if (status === "authenticated" && session) {
      const searchParams = new URLSearchParams(window.location.search);
      const key = searchParams.get("key");
      if (session.isNewUser) {
        message.success(`Welcome ${session.user?.name}`).then(() => {
          if (!key) {
            window.location.href = "/chatbot";
          } else {
  ...
  const register = async () => {
    ...
    setIsModalOpen(true);
    if (isCaptchaVerify) {
      // Registration logic (commented out)
    }
  };

  const checkEmail = (e: any) => {
    let emailVal: string = e?.target?.value.toLowerCase();
    setEmail(emailVal);
    if (emailVal === "") {
      setEmailMessage("Please enter email");
      return;
    ...
    const pattern = /^\w+([-+.']\w+)*@\w+([-.]\w+)*\.\w+([-.]\w+)*$/;
    const result = emailVal?.match(pattern);
    if (!result) {
      setEmailMessage("Invalid email address format");
    } else {
      setEmailMessage("");
  ...
  const checkPassword = (e: any) => {
    let passwordVal: string = e?.target?.value;
  ...
  const checkFirstName = (e: any) => {
    let firstNameVal: string = e?.target?.value;
  ...
  const checklastName = (e: any) => {
    let lastNameVal: string = e?.target?.value;
