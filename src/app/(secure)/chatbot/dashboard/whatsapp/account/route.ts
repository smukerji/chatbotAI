...
async function enableDisableToggle(req: NextRequest) {
  ...
  try {
    let whatsAppDetailsId:any = req.nextUrl.searchParams.get("whatsappDetailsId");
    const db = (await connectDatabase())?.db();
    const collection = db?.collection('whatsappbot_details');
    const foundResult = await collection?.findOne({ _id: new ObjectId(whatsAppDetailsId) });

    if (foundResult) {
      let result = await collection?.updateOne({ _id: new ObjectId(whatsAppDetailsId) }, { $set: { isEnabled: !foundResult.isEnabled } });
