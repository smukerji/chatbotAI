...
async function createVapiAssistant(req: NextRequest) {
  try {
    ...
    vapiData.voice.chunkPlan.punctuationBoundaries =
      vapiData.voice.chunkPlan.punctuationBoundaries.map(
        (item: any) => item.label
      );
    ...
    if (!voiceBotRecord) {
      ...
      let fileId = "";
      if (
        "tools" in vapiData.model &&
        "fileIds" in vapiData.model.tools[0].knowledgeBases[0] &&
        Array.isArray(vapiData.model.tools[0].knowledgeBases[0].fileIds)
      ) {
        fileId = vapiData.model.tools[0].knowledgeBases[0].fileIds[0];
      }
      if (voiceBotRecordVapiExist) {
        //check if the record is already exist then update the record
        const options = {
          method: "PUT",
          headers: {
        ...
        let vapiResponse = await fetch(
          `https://api.vapi.ai/assistant/${voiceBotRecordVapiExist.vapiAssistantId}`,
          options
        );
        ...
        //if the record is not exist then create the record
...
async function getSingleAssistantDataFromVapi(req: NextRequest) {
  let step = 1;
  try {
    const vapiAssiId: string = req.nextUrl.searchParams.get(
      "assistantId"
    ) as string;
    ...
    let vapiResponse = await fetch(
      `https://api.vapi.ai/assistant/${vapiAssiId}`,
      options
    );
    ...
    return {
      result: vapiResponseData,
      assistantLocalData: voiceBotRecordVapiExist,
    };
  } catch (error: any) {
