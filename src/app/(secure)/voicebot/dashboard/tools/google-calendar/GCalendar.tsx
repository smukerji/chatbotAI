...
//   const [loadingCheck, setLoadingCheck] = useState<boolean>(false);
//   const [loadingCreate, setLoadingCreate] = useState<boolean>(false);
//   const [disconnectingTool, setDisconnectingTool] = useState<TabAction | null>(null);
//   const [connectedTools, setConnectedTools] = useState<string[]>([]);
//   const [callPublishAssistant, setCallPublishAssistant] = useState<boolean>(false);
//   const [popupRef, setPopupRef] = useState<Window | null>(null);
...
//     const tool = TOOL_DB_MAP[action];
//     if (!assistantPublished || !userVerified) return;
//     setDisconnectingTool(action);

//     try {
//       const res = await fetch(
//         `/voicebot/dashboard/api/google/disconnect-tool?assistantId=${assistantId}&userId=${userId ?? cookies.userId}&tool=${tool}`,
//         { method: "POST" }
//       );
//       const data = await res.json();
//       if (data.success) {
...
//         <div className="gcal-tool-title">{title}</div>
//         <div className="gcal-tool-label">{TOOL_LABEL_MAP[action]}</div>
//         {loading || disconnectingTool === action ? (
//           <div className="gcal-tool-loading">
//             <Spin size="small" />
//           </div>
//         ) : !isConnected ? (
...
//               danger
//               style={{ marginTop: 8, fontWeight: 500, width: 180 }}
//             >
//               Disconnect
//             </Button>
//           </div>
