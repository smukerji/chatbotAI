...
const GCalendar: React.FC<GCalendarProps> = ({
  ...
  const voicebotDetails = voiceBotContextData?.state ?? {};
  console.log("voicebot details:",voicebotDetails);
  const storageKey = `publishedTools_${assistantId}`;
  const hasMountedKey = `hasMounted_${assistantId}`;

  // Helper to update model.toolIds in context
  const updateVoicebotToolIds = (tools: ToolsType[]) => {
    const publishedToolIds = tools
      .filter((t: ToolsType) => t.publish)
      .map((t: ToolsType) => TOOL_ID_MAP[t.tool]);
  ...
  // --- FIXED AUTO PUBLISH EFFECT ---
const normalizeTools = (tools: ToolsType[]) =>
  (tools || [])
    .filter(t => t.publish)
    .map(t => t.tool)
    .sort()
