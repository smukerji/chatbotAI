...
import {
  updateAssistantLastTrainedMetaDataService,
  updateAssistantLastUsedMetaDataService,
  updateAssistantNumberOfCallMetaDataService,
} from "./services/metadata-update-service";
...
function Dashboard() {
  ...
  useEffect(() => {
    ...
    if (voiceBotContextData?.assistantInfo?.assistantName) {
      voiceBotContextData.updateState(
        "name",
    ...
    if (!voiceBotContextData?.assistantInfo["vapiAssistantId"]) {
      ...
      try {
        ...
        if (assistantCreateResponseParse?.record) {
          ...
          getAssistantData(
            assistantCreateResponseParse.record?.vapiAssistantId
          );
        }
  ...
  const getTemplatePrompts = async (recordId: string) => {
    ...
    try {
      ...
      voicebotDetails["model"]["messages"][0]["content"] =
        assistantDataResponseParse?.prompts;
    } catch (error: any) {
    } finally {
      setLoading(false);
    }
  ...
  const getAssistantData = async (vapiAssiId: string) => {
    ...
    try {
      ...
      if (assistantDataResponseParse?.result) {
        ...
        let punctuationBoundaries =
          assistantData.voice.chunkPlan.punctuationBoundaries;
        // ["。","，"]
        ...
        let punctuationBoundariesArray =
          punctuationBoundaryPredefineValues.filter((item) =>
        ...
        assistantData.voice.chunkPlan.punctuationBoundaries =
          punctuationBoundariesArray;
        assistantData.firstMessageMode = vapiAssistanceData.firstMessageMode;
        assistantData.llmRequestDelaySeconds =
          vapiAssistanceData.llmRequestDelaySeconds;
        assistantData.responseDelaySeconds =
          vapiAssistanceData.responseDelaySeconds;
        assistantData.hipaaEnabled = vapiAssistanceData.hipaaEnabled;
        assistantData.silenceTimeoutSeconds =
          vapiAssistanceData.silenceTimeoutSeconds;
        assistantData.maxDurationSeconds =
          vapiAssistanceData.maxDurationSeconds;
        assistantData.backgroundSound = vapiAssistanceData.backgroundSound;
        assistantData.backchannelingEnabled =
          vapiAssistanceData.backchannelingEnabled;
        assistantData.backgroundDenoisingEnabled =
          vapiAssistanceData.backgroundDenoisingEnabled;
        assistantData.modelOutputInMessagesEnabled =
          vapiAssistanceData.modelOutputInMessagesEnabled;
        assistantData.name = vapiAssistanceData.name;
        assistantData.numWordsToInterruptAssistant =
          vapiAssistanceData.numWordsToInterruptAssistant;
        assistantData.voicemailDetection =
          vapiAssistanceData.voicemailDetection;
        ...
        if (
          "knowledgeBase" in vapiAssistanceData.model &&
          "fileIds" in vapiAssistanceData.model.knowledgeBase &&
          Array.isArray(vapiAssistanceData.model.knowledgeBase.fileIds)
        ) {
        ...
        if (
          analysysPlanPickData?.structuredDataSchema &&
          analysysPlanPickData?.structuredDataSchema?.properties
        ) {
          let propertiesData =
            analysysPlanPickData?.structuredDataSchema?.properties;
          if (Object.keys(propertiesData).length > 0) {
            const propertyArrayData = Object.entries(propertiesData).map(
              ([name, value]: [string, any], index: number) => ({
                id: index,
                name,
                type: value.type,
            ...
            assistantData.analysisPlan.structuredDataSchema.properties =
              propertyArrayData;
          } else {
        ...
        if (
          vapiAssistanceData?.messagePlan &&
          vapiAssistanceData?.messagePlan?.idleMessages
        ) {
          ...
          const idleMessageList = [
            ...
            {
              value: "8",
              label: "Is there something specific you're looking for?",
            },
            {
              value: "9",
              label: "I'm here to help with any questions you have.",
            },
          ];
          ...
          if (messagePlanPickData.length > 0) {
            ...
            assistantData.analysisPlan.messagePlan.idleMessages =
              idleMessageArray;
          }
        }
      ...
      updateAssistantNumberOfCallMetaDataService(
        assistantDataResponseParse?.assistantLocalData?.vapiAssistantId,
        assistantDataResponseParse?.assistantLocalData?._id
      );
    } catch (error: any) {
      console.log("error", error);
      message.error("Error while getting the assistant data");
  ...
  const muteCallHandler = () => {
    if (isMuted) {
      // if the call is muted then unmute the call
      vapi.setMuted(false);
      setIsMuted(false);
    } else {
      // if the call is unmute then mute the call
      vapi.setMuted(true);
      setIsMuted(true);
    }
  };
  ...
  console.log(
    " publish loading  value after publish",
          ...
          <div>
            ...
            <div className="header-description">
              <h4 className="description">
                Add your data sources to train your Voice agent
              </h4>
            </div>
          </div>
          <div className="header-right">
            <div className="button-container">
              <Button
                ...
                onClick={showAdditionalContentItemHandler}
              >
                  ...
                  >
                    <div className="delete-assistant-button-content">
                      <Image
                        alt=""
                        src={documentTrash}
                        className="delete-assistant-button-icon"
                      ></Image>
                      <span className="delete-assistant-button-text">
                        Delete
                      </span>
                    </div>
                  </Button>
          ...
          <div className="list-container">
            <ul className="tool-list">
              <li
                ...
                onClick={() => changeHandler("model")}
              >
                Model
              </li>
              <li
                className={tab == "transcriber" ? "active" : ""}
                onClick={() => changeHandler("transcriber")}
              >
                Transcriber
              </li>
              <li
                className={tab == "voice" ? "active" : ""}
                onClick={() => changeHandler("voice")}
              >
                Voice
              </li>
              {/* <li className={tab == "tool" ? "active" : ""} onClick={() => changeHandler("tool")}>Tool</li> */}
              <li
                className={tab == "advance" ? "active" : ""}
                onClick={() => changeHandler("advance")}
              >
                Advance
              </li>
              <li
                className={tab == "analysis" ? "active" : ""}
                onClick={() => changeHandler("analysis")}
              >
                Analysis
              </li>
              <li
                className={tab == "knowledge" ? "active" : ""}
                onClick={() => changeHandler("knowledge")}
              >
                Knowledge
              </li>

              <li
                className={tab == "phone-number" ? "active" : ""}
                onClick={() => changeHandler("phone-number")}
              >
                Phone Number
              </li>
              <li
                className={tab == "call-logs" ? "active" : ""}
                onClick={() => changeHandler("call-logs")}
              >
                Call Logs
              </li>
              <li
                className={tab == "embedding" ? "active" : ""}
                onClick={() => changeHandler("embedding")}
              >
                Embed on site
              </li>
              <li
                className={tab == "tools" ? "active" : ""}
                onClick={() => changeHandler("tools")}
              >
                Tools
              </li>
            </ul>
